...
class IRHandler:
    ...
    def update_ir_with_phi(irHandler):
        """ Updates the existing IR with inserted φ-functions while preserving original jump offsets. """
        if irHandler.cfg is None:
            print("CFG is not available, cannot update IR with φ-functions.")
            return

        old_ir = irHandler.ir  
        cfg = irHandler.cfg  
        irHandler.pretty_print(old_ir)
        new_ir = []  
        jump_offset_map = {idx: stmt[1] for idx, stmt in enumerate(old_ir)}

        for block in cfg:
            for instr, ir_index in block.instrlist:
                if isinstance(instr, PhiCommand):
                    new_ir.append((instr, 1))  # φ-functions always have jump offset 1
                else:
                    new_ir.append((instr, jump_offset_map.get(ir_index, 1)))  # Preserve original jump offsets

        irHandler.setIR(new_ir)
        print("\nUpdated IR with φ-functions:\n")
        irHandler.pretty_print(new_ir)
