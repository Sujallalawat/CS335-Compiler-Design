...
class ConcreteInterpreter(Interpreter):
    ...
    def interpret(self):
        ...
        if isinstance(stmt, ChironAST.PhiCommand):
            print("  Processing φ-function:", stmt)

            variable_name = stmt.variable.varname if isinstance(stmt.variable, ChironAST.Var) else str(stmt.variable)
            variable_name = variable_name.replace(":", "")

            # Select the maximum valid predecessor value
            valid_predecessors = []
            for pred in stmt.predecessors:
                if isinstance(pred, ChironAST.Var):
                    pred_var_name = pred.varname.replace(":", "")
                    if hasattr(self.prg, pred_var_name):
                        valid_predecessors.append(getattr(self.prg, pred_var_name))
                else:
                    valid_predecessors.append(pred)

            selected_value = max(valid_predecessors) if valid_predecessors else 0

            print(f"  Assigning {variable_name} = {selected_value} (from φ-function)")
            setattr(self.prg, variable_name, selected_value)

            self.pc += tgt  # Skip the φ-function execution
        ...
        elif isinstance(stmt, ChironAST.PhiCommand):
            ntgt = self.handlePhi(stmt, tgt)  # Handle φ-functions properly    
        else:
    ...
    def handlePhi(self, stmt, tgt):
        """ Handles execution of φ-functions by selecting the correct predecessor value. """
        print("  Phi Function Execution")

        variable_name = stmt.variable.varname if isinstance(stmt.variable, ChironAST.Var) else str(stmt.variable)
        variable_name = variable_name.replace(":", "")

        # Ensure that the variable exists in self.prg before assignment
        if not hasattr(self.prg, variable_name):
            print(f"  Initializing {variable_name} with default value 0 (from φ-function)")
            setattr(self.prg, variable_name, 0)  # Initialize with 0

        # Select the best available predecessor value
        valid_predecessors = []
        for pred in stmt.predecessors:
            if isinstance(pred, ChironAST.Var):
                pred_var_name = pred.varname.replace(":", "")
        ...
        # If there are valid predecessors, take the largest one (to preserve loop behavior)
        selected_value = max(valid_predecessors) if valid_predecessors else 0
